...
class FactorizedEmbedding(nn.Module):
    ...
    def __init__(self, factored_vocab_size: int, num_factored_vocabs: int, d_model: int, mask_token_id: int):
        ...
        self.factored_embeds = nn.ParameterList([nn.Embedding(factored_vocab_size, d_model) for _ in range(num_factored_vocabs)])
        self.mask_token_embed = nn.Parameter(torch.zeros(1, d_model))

    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        """

        Factorize the input token ids and sum the embeddings in each factorized vocabulary.

        Args:
            input_ids(torch.Tensor[torch.long]): Shape (B, T, H*W) tokens to be factorized

        Returns:
            input embeddings(torch.Tensor[torch.float]): Shape (B, T, H*W, d_model) additive factorized embeddings
        """
        ...
        factored_token_ids = factorize_token_ids(input_ids[is_not_mask], self.num_factored_vocabs, self.factored_vocab_size)

        unmasked_embeds = [
            factored_embed(factored_token_ids) for factored_embed, factored_token_ids in zip(self.factored_embeds, factored_token_ids.unbind(-1), strict=False)
        ]
...
def factorize_token_ids(token_ids: torch.Tensor, num_factored_vocabs: int = 2, factored_vocab_size: int = 512) -> torch.Tensor:
    """
    ...
    """
    assert token_ids.dtype is torch.long
...
def unfactorize_token_ids(factored_token_ids: torch.Tensor, num_factored_vocabs: int = 2, factored_vocab_size: int = 512) -> torch.Tensor:
    """
    ...
    """
    assert factored_token_ids.dtype is torch.long
...
def factorize_labels(labels_THW: torch.Tensor, num_factored_vocabs: int = 2, factored_vocab_size: int = 512) -> torch.Tensor:
    """
    ...
    """
    assert labels_THW.dtype is torch.long
