...
def test_model_v2(model, dataloader, optimizer,encoder, decoder, num_epochs=10, weight_path = "weights_w_cosmos.pt" ):
    ...
    for images, actions in dataloader:
        ...
        offset_input = {
        "actions": actions + random_offset,
        "observations": images
        }
        # Compute the loss
        predictions, loss = model.compute_training_loss_output(input, last_frame_only = True)
        predictions_o, loss_o = model.compute_training_loss_output(offset_input, last_frame_only = True)
...
def test_model(model, dataloader, optimizer, num_epochs=10, weight_path = "weights.pt"):
    if os.path.exists(weight_path):
        print(f"Loading weights from {weight_path}")
        model.load_state_dict(torch.load(weight_path))
    pred_steps = 5
    ...
    for epoch in range(num_epochs):
        ...
        for images, actions in dataloader:
            ...
            # Compute the loss
            predictions, loss = model.compute_training_loss_output(input, loss_frames = [3])
            predictions_o, loss_o = model.compute_training_loss_output(offset_input, loss_frames = [3])
...
if __name__ == "__main__":
    ...
    # Train the model
    if USECOSMOS:
        test_model_v2(model, dataloader, optimizer, encoder, decoder, num_epochs)
    else:
        test_model(model, dataloader, optimizer,  num_epochs)
