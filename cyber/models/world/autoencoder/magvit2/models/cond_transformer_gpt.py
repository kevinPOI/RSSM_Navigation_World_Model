...
from cyber.models.world.autoencoder.magvit2.modules.util import SOSProvider
...
class Net2NetTransformer(L.LightningModule):
    ...
    def state_dict(self, *kwargs, destination=None, prefix="", keep_vars=False):
        return {
            k: v
            for k, v in super().state_dict(*kwargs, destination, prefix, keep_vars).items()
    ...
    def init_cond_stage_from_ckpt(self, config):
        if config == "__is_first_stage__":
            ...
            print(f"Using no cond stage. Assuming the training is intended to be unconditional. " f"Prepending {self.sos_token} as a sos token.")
