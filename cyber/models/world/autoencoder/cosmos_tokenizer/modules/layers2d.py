...
from cyber.models.world.autoencoder.cosmos_tokenizer.modules.patching import Patcher, UnPatcher
from cyber.models.world.autoencoder.cosmos_tokenizer.modules.utils import Normalize, nonlinearity
...
class Encoder(nn.Module):
    def __init__(
        ...
        **ignore_kwargs,
        ...
        assert self.num_downsamples <= self.num_resolutions, f"we can only downsample {self.num_resolutions} times at most"

        # downsampling
        self.conv_in = torch.nn.Conv2d(in_channels, channels, kernel_size=3, stride=1, padding=1)

        curr_res = resolution // patch_size
        in_ch_mult = (1,) + tuple(channels_mult)  # noqa: RUF005
